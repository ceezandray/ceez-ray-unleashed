...
import { ArrowLeft, Plus, X, Columns3, MessageSquare, GripVertical, Upload, ChevronDown, ChevronRight } from "lucide-react";
...
const StoryboardPage = () => {
  ...
  const handleImageUpload = (files: FileList | null, sceneId: string) => {
    ...
    setEpisodes(prev => prev.map(ep => {
      ...
      return {
        ...ep,
        scenes: ep.scenes.map(s => s.id === sceneId ? { ...s, images: [...s.images, ...newImages] } : s),
      };
    }));
  };
  ...
  const removeImage = (sceneId: string, imgId: string) => {
    setEpisodes(prev => prev.map(ep => {
      ...
      return {
        ...ep,
        scenes: ep.scenes.map(s => s.id === sceneId ? { ...s, images: s.images.filter(img => img.id !== imgId) } : s),
      };
    }));
  };
  ...
  return (
    <div className="dash-font h-screen flex flex-col bg-[hsl(var(--dash-bg))] text-[hsl(var(--dash-text))] overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {[3, 4, 5].map(n => (
            <button
              ...
              className={`w-8 h-8 rounded-lg text-xs font-bold border transition-all ${columns === n ? "bg-[#2196f3] border-[#2196f3] text-white" : "border-[rgba(255,255,255,0.18)] text-[hsl(var(--dash-text-3))] hover:border-[#2196f3]"}`}
            >
              {n}
            </button>
          ))}
        ...
        <div className="w-[220px] bg-[hsl(var(--sidebar-background))] border-r border-[rgba(255,255,255,0.09)] overflow-y-auto flex-shrink-0 p-3">
          ...
          {episodes.map(ep => (
            <button
              ...
              className={`flex items-center gap-2 w-full px-3 py-2.5 rounded-[10px] text-sm font-semibold transition-all border mb-1 ${
                activeEp === ep.id
                  ? "bg-[#2196f3] text-white border-transparent shadow-[0_4px_16px_rgba(33,150,243,0.3)]"
                  : "text-[hsl(var(--dash-text-3))] border-transparent hover:bg-[rgba(33,150,243,0.08)]"
              }`}
            >
          ...
          {activeEpisode && (
            <>
              <div className="flex items-center justify-between mb-5">
                ...
                <button
                  onClick={() => addScene(activeEp)}
                  className="flex items-center gap-2 px-4 py-2.5 bg-[#2196f3] rounded-xl text-sm font-bold text-white hover:bg-[#1976d2] transition-all shadow-lg"
                >
              ...
              {activeEpisode.scenes.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  <div className="w-16 h-16 rounded-2xl bg-[rgba(33,150,243,0.1)] flex items-center justify-center mb-4">
                    <Columns3 size={28} className="text-[#2196f3]" />
                  </div>
              ...
              {activeEpisode.scenes.map(scene => (
                <div key={scene.id} className="mb-6 bg-[hsl(var(--dash-surface))] border border-[rgba(255,255,255,0.09)] rounded-[14px] overflow-hidden shadow-lg">
                  ...
                  <div className="flex items-center justify-between px-4 py-3 border-b border-[rgba(255,255,255,0.09)] bg-[hsl(var(--dash-surface2))]">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => { setUploadSceneId(scene.id); fileInputRef.current?.click(); }}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[rgba(255,255,255,0.18)] text-xs font-semibold text-[hsl(var(--dash-text-3))] hover:border-[#2196f3] hover:text-[#2196f3] transition-all"
                      >
                        <Upload size={13} /> Upload Images
                      </button>
                      <button
                        onClick={() => deleteScene(scene.id)}
                        className="w-7 h-7 rounded-lg border border-[rgba(255,255,255,0.18)] flex items-center justify-center text-[hsl(var(--dash-text-4))] hover:border-[hsl(var(--dash-red))] hover:text-[hsl(var(--dash-red))] transition-all"
                      >
                        <X size={13} />
                      </button>
                    </div>
                  ...
                  <div className="p-4">
                              ...
                              <Draggable key={img.id} draggableId={img.id} index={idx}>
                                {(provided, snapshot) => (
                                  ...
                                  >
                                    ...
                                    <button
                                      onClick={() => removeImage(scene.id, img.id)}
                                      className="absolute top-1 right-1 z-10 w-6 h-6 rounded-full bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-[hsl(var(--dash-red))]"
                                    >
                    ...
                    {scene.images.length === 0 && (
                      <div
                        onClick={() => { setUploadSceneId(scene.id); fileInputRef.current?.click(); }}
                        className="flex flex-col items-center justify-center py-10 border-2 border-dashed border-[rgba(255,255,255,0.12)] rounded-xl cursor-pointer hover:border-[#2196f3] transition-colors"
                      >
                  ...
                  <div className="px-4 pb-4 border-t border-[rgba(255,255,255,0.06)] pt-3">
                    ...
                    {scene.comments.map(c => (
                      <div key={c.id} className="flex gap-2 mb-2">
                        <div className="w-6 h-6 rounded-full bg-[#2196f3] flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0 mt-0.5">
                          {c.user[0].toUpperCase()}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold">{c.user}</span>
                            <span className="text-[10px] text-[hsl(var(--dash-text-4))]">{c.date}</span>
                          </div>
      ...
      <input
        ...
        multiple
        className="hidden"
        onChange={e => { if (uploadSceneId) handleImageUpload(e.target.files, uploadSceneId); e.target.value = ""; }}
      />
    </div>
  );
};
